...
const Movies = () => {
  ...
  const getData = async () => {
    try {
      const res = await axios.get(`/movie/${filter}?page=${page}`);
    ...
    <div className="w-full text-white min-h-screen bg-[#1D1C23] p-5">
      ...
      >
        <div className="w-full flex justify-center  flex-wrap gap-12 pt-20 ">
          {trending.map((e) => (
            <div onClick={()=>navigate(`/movie/${e.id}`)} className="w-[18%] relative h-[365px] text-[21.5px] font-semibold rounded overflow-hidden">
              <img
                ...
                alt="not showing"
              />
              <h1 className="mt-2 opacity-60">
                {e.name || e.title || e.original_name || e.original_title}
              </h1>
              <div className="w-12 flex items-center justify-center absolute bottom-[20%] -right-[0%] h-12 rounded-full text-white bg-yellow-600">
